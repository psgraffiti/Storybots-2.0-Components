...
function playList(options,arg){
  ...
  player.pl._nextPrev = function(func){
    var playlistEnd, playlistStart, addendum, newIndex;

    if (func === 'next'){
      playlistStart = 0;
      playlistEnd   = player.pl.videos.length -1;
      addendum      = 1;
    }
    else {
      playlistStart = player.pl.videos.length -1;
      playlistEnd   = 0;
      addendum      = -1;
    }

    if (player.pl.current !== playlistEnd) {
      newIndex = player.pl.current + addendum;
    }
    else {
      newIndex = playlistStart;
    }

    player.pl._setVideo(newIndex);
    player.trigger(func, [player.pl.videos[newIndex]]);
    $('.vjs-title').html(player.pl.videos[newIndex].title)
  };
  ...
  player.pl._setVideo = function(index){
    if (index < player.pl.videos.length){
      ...
      $('.vjs-title').html(player.pl.videos[index].title)
  ...
  player.pl._videoEnd = function() {
    // Make sure the playlist starts over when last video is finished
